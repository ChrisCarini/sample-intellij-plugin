...
export async function updateGradleProperties(new_version: semver.SemVer): Promise<semver.SemVer> {
  try {
    ...
    const obj: JBGradlePropertiesFile = properties.parse(readFileSync(gradle_file, 'utf-8'), {
      namespaces: true,
      sections: true,
      variables: true,
    });
    ...
    const current_plugin_verifier_ide_versions = parseSemver(obj?.pluginVerifierIdeVersions);
    ...
    const result = data
      ...
      .replace(
        new RegExp(`^pluginVerifierIdeVersions = ${current_plugin_verifier_ide_versions}$`, 'gm'),
...
export async function updateChangelog(new_version: semver.SemVer): Promise<void> {
  try {
